...
- **sprint-step-skip-late**: Steps ❿ PR+CI skipped under context pressure. Model treats G3 PASS + commit as "done" and emits COMPLETE without PR. Causes: no explicit non-skippability constraint; ❿ is 10th step (late = higher skip risk); output format has no session-state signal. Fix: SKILL.md 4-point positional redundancy (argument parsing, step ❿.8, output format, constraints). Occurred: observed pattern. Hardened: SKILL.md reinforcement
